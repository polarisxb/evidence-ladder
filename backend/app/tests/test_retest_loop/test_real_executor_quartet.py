...
def _task():
    return types.SimpleNamespace(id="scan1", runtime_vars={}, target_type="adapter",
                                 target_config={}, _resolved_adapter_payload={"enabled": True})


def _variants(with_controls=True):
    attack = {"variant_type": "attack", "is_primary": True,
              "response_text": "ok", "response_status": "completed", "latency_ms": 100.0}
    if not with_controls:
        return [attack]
    controls = [
        {"variant_type": v, "response_text": "x", "response_status": "completed", "latency_ms": 50.0}
...
def _patch(monkeypatch, *, variants, control_assessment, case_summary):
    ...
    async def _fake_analyze(task, template, case_variants, **kwargs):
        return {
            "control_summary": {"control_assessment": control_assessment,
                                "control_summary": "..."},
...
def test_quartet_attack_delta_promotes_to_rule_evidence(monkeypatch):
    _patch(monkeypatch, variants=_variants(), control_assessment="attack_delta_supported",
           case_summary={"tool_observed": False, "tool_calls": []})
    delta = asyncio.run(RealRetestExecutor(_task()).run_quartet({"case_id": "c1", "payload_text": "p"}))
...
def test_quartet_attack_only_tool_promotes_to_tool_observed(monkeypatch):
    _patch(monkeypatch, variants=_variants(), control_assessment="attack_delta_supported",
           case_summary={"tool_observed": True, "tool_calls": [{"name": "forward_email"}],
...
def test_quartet_control_also_succeeds_is_contradiction(monkeypatch):
    _patch(monkeypatch, variants=_variants(), control_assessment="discussion_supported",
...
def test_quartet_controls_inconclusive_is_contradiction(monkeypatch):
    _patch(monkeypatch, variants=_variants(), control_assessment="controls_inconclusive",
           case_summary={"tool_observed": False})
    delta = asyncio.run(RealRetestExecutor(_task()).run_quartet({"case_id": "c1", "payload_text": "p"}))
    assert delta.contradiction is True
...
def test_quartet_controls_missing_is_noop(monkeypatch):
    _patch(monkeypatch, variants=_variants(with_controls=False),
           control_assessment="controls_missing", case_summary={"tool_observed": False})
    delta = asyncio.run(RealRetestExecutor(_task()).run_quartet({"case_id": "c1", "payload_text": "p"}))
    assert delta.contradiction is False
    assert delta.evidence_updates == {}
