...
- 模型数量：1-2 个。
- 标注数量：至少 80 条结果（与 §15 第 8 条 MVP 边界对齐）。

目标是验证评测链路能否跑通，并初步比较 Judge ASR、Text-Claim ASR 和 Probe-Verified ASR 的差异。

### 9.2 正式实验

本节采用**双轨设计**：**大创结题 MVP 版**作为最低交付（与 §15 第 8 条对齐），**完整论文版**作为 v0.5+ 扩展目标。两轨共享相同的实验协议、Quartet 设计与统计要求。

**(a) 大创结题 MVP 版**（Stage 4 邮件 Agent 沙箱阶段交付）：

- 场景：1 个 Agent 场景（邮件 Agent，覆盖 read / send / delete + outbox 状态）。
- 逻辑用例：50 个（含 Pilot 30 + 正式扩展 20）。
- 变体数量：每用例 4 个 Quartet 变体，含 10% 重复运行 → 约 220 次运行。
- 模型数量：2 个（推荐 GPT-4o-mini + DeepSeek-V3）。
- 人工标注：100 条双人盲标。

**(b) 完整论文版**（v0.5+ 扩展目标）：

- 场景：2-3 个应用 / Agent 场景（追加电商 / 工单系统）。
- 逻辑用例：100 个以上。
...
- **MVP 版至少 100 条、完整版至少 200 条**结果由两名标注者独立标注，标注字段至少包括：是否成功、是否仅讨论、是否越权声称、是否触发工具调用、是否被 Probe 验证、原任务是否完成。
...
- MVP 正式版扩展至 50 用例（运行约 220）时，重新计算 power 并按需调整最小可检测效应；完整版扩展至 100+ 用例时同步披露。
...
> **进度采用滚动式管理**：以上 6 阶段为大方向；每阶段结束后按 [`docs/dev-notes/roadmap.zh-CN.md`](./dev-notes/roadmap.zh-CN.md) §7 复盘机制评估完成度、数据合理性与资源消耗，必要时按 §8 调整原则对后续阶段进行微调。Stage 1 详细执行计划另见 [`docs/dev-notes/stage1_plan.zh-CN.md`](./dev-notes/stage1_plan.zh-CN.md)，与本进度表中的"第 1-2 周"对应。两份文档已随 commit `992c303` 公开发布，可在线访问 <https://github.com/polarisxb/evidence-ladder/tree/main/docs/dev-notes>。
