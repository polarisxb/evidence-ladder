...
- **千问云**（qianwenai.com / 百炼）是阿里正门，可支付宝；可当 T1 一厂 Qwen 或硅基备用。**不要**用一把阿里云 key 打完整张 5 家族矩阵（serving 不独立）。密钥只在本机 Settings / `DASHSCOPE_API_KEY` 登记，**不要发到聊天**。见 `provider-setup.v2.md` §1b
