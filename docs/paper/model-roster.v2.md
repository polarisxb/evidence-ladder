...
因此闭源位用厂商**当前默认 SKU**（Kimi K3、GLM-5.3），开源位用可复托管的权重（Qwen3.8-27B、DeepSeek-V3.1）。一边代表「现在还在卖的系统」，一边保证别人能复核。
...
| 角色 | API ID | 平台 | 档位 |
|------|--------|------|------|
| Target-1 | `qwen3.8-27b` | 千问云 / 百炼（支付宝） | 一厂开源锚点（现行 ~27B 档，不追 max/2.4T） |
...
| 日期 | 变更 |
|------|------|
| 2026-09-01 | v2.4 T1 刷新：直播千问云目录 → `qwen3.8-27b`（一厂）；Kimi/GLM/DeepSeek/MiniMax 仍不走这把阿里云 key |
